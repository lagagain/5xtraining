...
- [ ] 利用 [[https://github.com/creationix/nvm][nvm]] 或
   [[https://brew.sh/index_zh-tw][Homebrew]] 安裝最新版本的 Node.js
- [ ] 以 =node -v= 指令來確認 Node.js 的版本

**** 1-2: 安裝 [[https://yarnpkg.com/zh-Hans/][yarn]][%]
     :PROPERTIES:
     :CUSTOM_ID: 安裝-yarn
     :END:
...
- [ ] 了解 =dependencies=、=devDependencies=、=peerDependencies=
   的差異與指令

*** 步驟 2: 在 GitHub 建立 repo[0%]
    :PROPERTIES:
...
- [ ] 在 =create-react-app= 產生的專案目錄下，建立 =docs= [%]
   資料夾，並將本教程文件 commit 進去

   - [ ] 搬進來是為了方便之後開發時可以參考

- [ ] 將成品 push 到 GitHub 的 repo
- [ ] 將使用的 Node.js 版號寫進 =package.json=（也請確認 React
   版號是否有標明）

*** 步驟 4: 復刻 [[https://5xruby.tw/][五倍官網首頁]][%]
    :PROPERTIES:
    :CUSTOM_ID: 步驟 4-復刻-五倍官網首頁
    :END:

- [ ] 引入 =bootstrap=、=jquery= 請使用 yarn 安裝使用
- [ ] 不使用 React、其它 library, 只能用 jquery + css
...
*** 步驟 5: 使用 React 改寫 [[https://5xruby.tw/][五倍官網首頁]][%]
    :PROPERTIES:
